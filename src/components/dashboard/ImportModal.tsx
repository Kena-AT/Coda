    ...
    <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-4 backdrop-blur-xl">
      ...
      <div className="absolute inset-0 pointer-events-none opacity-5">
        <div className="w-full h-full flex flex-wrap">
          {[...Array(64)].map((_, i) => (
            <div key={i} className="w-1/8 h-1/8 border border-[#e60000]/20" />
          ))}
        </div>
      </div>
      ...
      <div className="bg-[#0e0e0e] border border-[#222226] w-full max-w-[1024px] h-[720px] shadow-[20px_20px_0px_#00000080] relative flex flex-col overflow-hidden">
        ...
        <div className="h-14 bg-[#1a1a1a] border-b border-[#353534]/50 flex items-center justify-between px-6 shrink-0 mr-1 mt-1">
          ...
          <button onClick={onClose} className="p-2 text-[#adaaad] hover:text-[#e60000] transition-colors">
            <X size={18} />
          </button>
        </div>
          ...
          <aside className="w-64 border-r border-[#353534]/30 bg-[#111111] flex flex-col justify-between">
            <div className="flex flex-col p-4 gap-1">
              ...
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-[10px] font-main font-bold uppercase tracking-[1px] transition-all border-l-2 ${activeTab === 'PASTE_CODE' ? 'bg-[#e60000]/10 border-[#e60000] text-white' : 'border-transparent text-[#adaaad] hover:text-white hover:bg-white/5'}`}
              >
                <Code2 size={14} />
                PASTE_CODE
              </button>
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-[10px] font-main font-bold uppercase tracking-[1px] transition-all border-l-2 ${activeTab === 'UPLOAD_FILES' ? 'bg-[#e60000]/10 border-[#e60000] text-white' : 'border-transparent text-[#adaaad] hover:text-white hover:bg-white/5'}`}
              >
                <UploadCloud size={14} />
                UPLOAD_FILES
              </button>
          ...
          <main className="flex-1 flex flex-col bg-[#0e0e0e] overflow-hidden">
              ...
              <div className="flex-1 flex flex-col p-8 overflow-y-auto custom-scrollbar">
                ...
                <div className="flex justify-between items-end mb-6">
                   ...
                   <div className="bg-[#1a1a1a] border border-[#353534] px-4 py-1 flex items-center gap-2">
                       ...
                       <span className="text-[10px] font-mono text-[#e60000] font-bold uppercase">{predictedLang}</span>
                   </div>
                </div>
                ...
                <div className="mt-6 flex gap-6">
                   <div className="flex-1 flex flex-col gap-4">
                      <button 
                        ...
                        className="w-full py-3 border border-[#353534] bg-[#1a1a1a] text-white text-[10px] font-main font-bold uppercase tracking-[1.5px] hover:border-[#e60000] hover:text-[#e60000] transition-colors flex items-center justify-center gap-2"
                      >
                        <Cpu size={14} />
                        GENERATE_METADATA
                      </button>
                      ...
                      <div className="bg-[#151515] border border-[#222226] p-4 min-h-[100px]">
                         ...
                         <div className="flex flex-wrap gap-2">
                            {predictedTags.length > 0 ? predictedTags.map(tag => (
                              <span key={tag} className="px-2 py-1 bg-[#e60000]/10 border border-[#e60000]/30 text-[#e60000] text-[10px] font-mono uppercase">#{tag}</span>
                            )) : (
                   ...
                   <div className="flex-1 bg-[#151515] border border-[#222226] p-4 flex flex-col relative overflow-hidden">
                      {/* Decorative Element */}
                      <div className="absolute -bottom-4 -right-4 w-20 h-20 border border-[#e60000]/10 rounded-full" />
                      ...
                      <div className="flex flex-col gap-1 flex-1 overflow-y-auto max-h-[160px] custom-scrollbar relative z-10">
                         <button 
                           ...
                           className={`w-full text-left p-2 text-[10px] font-mono uppercase tracking-[0.5px] transition-colors flex items-center justify-between ${selectedProjectId === null ? 'bg-[#e60000] text-white' : 'text-[#adaaad] hover:bg-white/5'}`}
                         >
                         ...
                         {projects.map(p => (
                           <button 
                             ...
                             className={`w-full text-left p-2 text-[10px] font-mono uppercase tracking-[0.5px] transition-colors flex items-center justify-between ${selectedProjectId === p.id ? 'bg-[#e60000] text-white' : 'text-[#adaaad] hover:bg-white/5'}`}
                           >
            ...
            <div className="h-20 border-t border-[#353534]/30 bg-[#161616] flex items-center justify-between px-8 shrink-0">
               <div className="flex flex-col">
                  ...
                  <p className="text-[#e60000] text-[8px] font-mono uppercase opacity-60 mt-1">Authorized personnel only</p>
               </div>
               <div className="flex items-center gap-6">
                 ...
                 <button 
                  ...
                  className="bg-[#e60000] text-white text-[11px] font-main font-bold uppercase tracking-[2px] px-10 py-3.5 hover:bg-[#ff0000] transition-all shadow-[8px_8px_0px_#7a0000]"
                 >
                   {isProcessing ? 'PROCESSING...' : 'IMPORT_SNIPPET'}
                 </button>
               </div>
            </div>
